...
const InternalHeader: React.FC = () => {
  ...
  // ✅ Only show Profile icon inside Employee Portal routes
  ...
  const handleProfileClick = () => {
    // ✅ Employee only (and only shown in employee portal anyway)
  ...
  return (
    <header className="bg-white dark:bg-gray-800 shadow-sm sticky top-0 z-40 h-16 flex items-center justify-between px-4 sm:px-6">
      ...
      <div className="flex items-center">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="text-xl font-bold text-gray-900 dark:text-white hidden sm:block">
            Spazaafy{' '}
            <span className="text-xs font-normal text-gray-500 uppercase ml-1">Internal</span>
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-4">
        ...
        {/* ✅ Profile icon only for employee portal */}
        {isEmployeePortal && (
          <button
