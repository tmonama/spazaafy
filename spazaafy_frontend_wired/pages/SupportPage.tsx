...
const SupportPage: React.FC = () => {
  ...
  // Determine User Type
  const isInternal = ['admin', 'hr', 'legal', 'employee'].includes(
    (user?.role?.toLowerCase() || '')
  );

  // Detect portal prefix from URL so links stay inside the right portal
  // e.g. /hr/support -> "hr"
  const portalPrefix = (() => {
    const first = window.location.pathname.split('/')[1]?.toLowerCase();
    if (['admin', 'hr', 'legal', 'employee', 'tech'].includes(first)) return first;
    // fallback from role
  ...
  const fetchTickets = async () => {
    try {
      ...
      if (isInternal && token) {
        ...
        const myTickets = data.filter((tt: any) =>
          tt.requester === user?.id ||
          (user?.firstName && (tt.requester_name || '').includes(user.firstName))
        );
        ...
        fetchedTickets.sort((a: any, b: any) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isInternal && token) {
        ...
        // ✅ Submit to Customer Support (mock)
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-2 order-2 md:order-1">
            <Card title={isInternal ? "My Tech Requests" : t('supportPage.yourTickets')}>
              {loading ? (
                <p className="text-center py-4 text-gray-500">{t('supportPage.loading')}</p>
              ) : error ? (
                ...
                <div className="space-y-4">
                  {tickets.length === 0 ? (
                    ...
                    tickets.map(ticket => {
                      // ✅ IMPORTANT: Internal users should stay inside their portal
                      // Example: /hr/support/tech/:id instead of /tech/tickets/:id
                      const detailLink = isInternal
                        ? `/${portalPrefix}/support/tech/${ticket.id}`
                        : `/support/${ticket.id}`;
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {['IT_SUPPORT', 'ACCESS', 'BUG', 'REFERRAL'].map((cat) => (
                        <div
                          ...
                          className={`cursor-pointer border rounded-md p-2 text-center text-xs font-bold transition ${newTicket.category === cat
