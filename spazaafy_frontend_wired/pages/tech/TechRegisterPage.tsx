...
const TechRegisterPage: React.FC = () => {
  ...
  const handleUpgrade = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await mockApi.accessControl.upgradeUser(email, code, 'TECH');
      
      alert(res.detail || 'Access granted. Redirecting to login...');
      navigate('/tech/login');
    } catch (err: any) {
        ...
        <div className="bg-[#1e1e1e] border border-gray-800 rounded-2xl p-8 shadow-2xl">
          ...
          {step === 1 ? (
            <form onSubmit={handleRequestCode} className="space-y-6">
              <div className="bg-blue-900/20 border border-blue-900/50 text-blue-300 px-4 py-3 rounded-lg text-sm mb-4">
                You must already have an active <strong>Employee Profile</strong> to request Tech access.
              </div>
              <div>
